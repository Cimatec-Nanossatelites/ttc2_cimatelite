...
static int spi_setup_gpio(spi_port_t port)
{
    ...
    switch(port)
    {
        ...
        case SPI_PORT_2:
            ...
            gpio_init(GPIO_PIN_66, conf);

            /* Set all CS pins to high */
            gpio_set_state(GPIO_PIN_66, true);
...
int spi_select_slave(spi_port_t port, spi_cs_t cs, bool active)
{
    ...
    switch(port)
    {
        ...
        case SPI_PORT_2:
            switch(cs)
            {
                case SPI_CS_0:      gpio_set_state(GPIO_PIN_66, !active);     break;
                case SPI_CS_NONE:                                             break;
                default:
                #if defined(CONFIG_DRIVERS_DEBUG_ENABLED) && (CONFIG_DRIVERS_DEBUG_ENABLED == 1)
                    sys_log_print_event_from_module(SYS_LOG_ERROR, SPI_MODULE_NAME, "Error selecting a slave from port 2: Invalid CS pin!");
                    sys_log_new_line();
                #endif /* CONFIG_DRIVERS_DEBUG_ENABLED */
        ...
        case SPI_PORT_4:
            ...
            break;
        case SPI_PORT_5:
            /* TODO: Define the CS pins of port 5 */
            break;
        default:
        #if defined(CONFIG_DRIVERS_DEBUG_ENABLED) && (CONFIG_DRIVERS_DEBUG_ENABLED == 1)
